...
class _AdminSidebarDashboardState extends State<AdminSidebarDashboard> {
  ...
  final List<Map<String, dynamic>> menuItems = [
    ...
    {'title': 'Notifications', 'icon': Icons.notifications_outlined, 'filter': 'notifications'},
  ...
  Future<void> _fetchUnreadCount() async {
    try {
      final response = await http.get(
        Uri.parse("http://localhost:8000/api/admin/notifications/count/?admin_type=${widget.adminRole}"),
      );
      if (response.statusCode == 200 && mounted) {
        final data = jsonDecode(response.body);
  ...
  Widget _sidebarAvatar() {
    ...
    return Container(
      ...
      child: hasPic
          ? Image.network(absoluteUrl!, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Icon(getRoleIcon(), color: Colors.white, size: 36))
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      Text(widget.adminName, style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold), maxLines: 1, overflow: TextOverflow.ellipsis),
                      const SizedBox(height: 4),
                      Text(getRoleTitle(), style: TextStyle(color: Colors.white.withAlpha(179), fontSize: 12)),
                    ],
                  ),
                ),
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final badgeCount = item['filter'] == 'notifications' ? _unreadNotifCount : 0;
                      ...
                      return Material(
                        ...
                        child: ListTile(
                          leading: Stack(
                            ...
                            children: [
                              Icon(item['icon'] as IconData, color: isSelected ? getRoleColor() : Colors.grey.shade500, size: 22),
                              if (badgeCount > 0)
                                Positioned(
                                  top: -4,
                                  right: -6,
  ...
  Widget _sidebarStat(String title, String value, IconData icon, Color color) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Row(children: [Icon(icon, color: color, size: 18), const SizedBox(width: 8), Text(title, style: TextStyle(color: color, fontWeight: FontWeight.w600, fontSize: 13))]),
