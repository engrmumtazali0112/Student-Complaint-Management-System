...
class AdminRejectedComplaintsScreen extends StatefulWidget {
  final String adminType;

  const AdminRejectedComplaintsScreen({super.key, required this.adminType});

  @override
  State<AdminRejectedComplaintsScreen> createState() => _AdminRejectedComplaintsScreenState();
}

class _AdminRejectedComplaintsScreenState extends State<AdminRejectedComplaintsScreen> {
  List complaints = [];
  bool isLoading = true;
  String searchQuery = "";
  ...
  Future<void> fetchRejectedComplaints() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('http://127.0.0.1:8000/api/admin/complaint/rejected/?admin_type=${widget.adminType}'),
      );
      ...
      child: Row(
        children: [
          ...
          Column(
            ...
            children: [
              const Text("Rejected Complaints", style: TextStyle(color: Colors.white70, fontSize: 14)),
    ...
    return Expanded(
      child: isLoading
          ...
          : filteredComplaints.isEmpty
              ? const Center(child: Text("No rejected complaints"))
              : ListView.builder(
                  padding: const EdgeInsets.all(16),
            ...
            Row(
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(color: const Color(0xFFEF4444).withAlpha(26), borderRadius: BorderRadius.circular(12)),
                  child: const Text("REJECTED", style: TextStyle(color: Color(0xFFEF4444), fontSize: 11)),
                ),
              ],
            ),
            const SizedBox(height: 8),
            Text("ID: #${complaint['id']} - ${complaint['student_name']} (${complaint['student_id']})"),
            Text("Reason: ${complaint['rejection_remarks']}", style: const TextStyle(fontSize: 12, color: Colors.red)),
